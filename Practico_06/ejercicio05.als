...
pred Asimetrica[r: Relacion]{
	r.rel in ~((r.elem->r.elem) - r.rel)
}
...
assert OrdenParcial_PrimerElemento{
	...
	all r: Relacion |
	some x: r.elem | (OrdenParcial[r] implies PrimerElemento_v1[r, x])
}
...
assert OrdenTotal_PU{
	all r: Relacion |
	some x, y: r.elem | (OrdenTotal[r]  and
								  PrimerElemento_v1[r, x] and
                               	       UltimoElemento_v1[r, y]) implies (x != y)
}
...
assert Union_OrdenesEstrictos{
	all r1, r2: Relacion | (OrdenEstricto[r1]  and
								   OrdenEstricto[r2]) implies
	(all r3: Relacion | (r3.elem = r1.elem + r2.elem and
							    r3.rel = r1.rel + r2.rel) implies OrdenEstricto[r3])
}
